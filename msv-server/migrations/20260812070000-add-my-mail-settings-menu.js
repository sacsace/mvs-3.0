  ...
  async up(queryInterface) {
    ...
    for (const parent of parents) {
      ...
      const [existing] = await sequelize.query(
        ...
        WHERE tenant_id = $1 AND route = '/my/mail-settings'
        LIMIT 1
        `,
        { bind: [tenantId] }
      );
      ...
      if (existing.length > 0) {
        ...
        const [ins] = await sequelize.query(
          ...
          INSERT INTO menus (
            ...
            $1, $2, $3, $4, '/my/mail-settings', $5, 7, 1, true, $6, NOW(), NOW()
          )
          RETURNING id
          `,
          {
  ...
  async down(queryInterface) {
    ...
    await sequelize.query(`
      DELETE FROM user_permissions WHERE menu_id IN (
        SELECT id FROM menus WHERE route = '/my/mail-settings'
      )
    `);
    await sequelize.query(`
      DELETE FROM menus WHERE route = '/my/mail-settings'
    `);
  },
};
