...
/** SAP Import Wizard 메뉴를 회계관리 하위에 등록하고 상위 메뉴 권한을 초기 복사한다. */
const MENU = {
  route: '/accounting/sap-import',
  nameKo: 'SAP Data 불러오기',
  nameEn: 'Load SAP Data',
  icon: 'upload_file',
  order: 2,
  description: 'SAP Excel/CSV 검증 및 MVS 임시 전표 변환',
};
...
module.exports = {
    ...
    for (const parent of parents) {
      ...
      if (!menuId) {
        const [inserted] = await queryInterface.sequelize.query(
          ...
          {
            bind: [
              parent.tenant_id, parent.id, MENU.nameKo, MENU.nameEn, MENU.route,
              MENU.icon, MENU.order, MENU.description,
            ],
          }
        );
        ...
        await queryInterface.sequelize.query(
          `UPDATE menus
           ...
           WHERE id = $7`,
          { bind: [parent.id, MENU.nameKo, MENU.nameEn, MENU.icon, MENU.order, MENU.description, menuId] }
        );
      }
      ...
      for (const permission of parentPermissions) {
        await queryInterface.sequelize.query(
          ...
          {
            bind: [
              permission.user_id, menuId, permission.can_view, permission.can_create,
  ...
  async down(queryInterface) {
    await queryInterface.sequelize.query(`
      ...
      WHERE route = '/accounting/sap-import'
    `);
  },
};
