...
export const runTallyImport = async (req: RequestWithUser, res: Response) => {
  ...
  try {
    ...
    const file = getUploadedFile(req);
    const { content, fileName, encoding, size } = readUploadText(file);
    const body = req.body || {};

    console.log('[tally/import] start', {
      fileName,
      encoding,
      size,
      companyId,
    ...
    const result = await importTallyExport(
      content,
      {
        ...
        importLedgers: parseBool(body.importLedgers, true),
        importVouchers: parseBool(body.importVouchers, true),
