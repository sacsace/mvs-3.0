...
export const resolveDepreciationRate = (input: DepreciationInput): number => {
  ...
  if ((input.depreciationMethod || 'straight_line') === 'declining_balance') {
    ...
    return round2(Math.min(100, (2 / life) * 100));
  }
...
export const applyDepreciationToAssetPayload = (body: Record<string, any>) => {
  const summary = calculateDepreciation({
    purchasePrice: Number(body.purchase_price || 0),
    salvageValue: Number(body.salvage_value || 0),
    usefulLife: Number(body.useful_life || 0),
    depreciationRate: Number(body.depreciation_rate || 0),
    purchaseDate: body.purchase_date,
    depreciationMethod: body.depreciation_method || 'straight_line',
  });

  return {
    ...body,
    salvage_value: summary.salvageValue,
    depreciation_rate: summary.depreciationRate,
    current_value: summary.currentValue,
    accumulated_depreciation: summary.accumulatedDepreciation,
  };
};
