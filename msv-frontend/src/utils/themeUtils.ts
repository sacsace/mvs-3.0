...
> = {
  light: {
    ...
    workArea: { main: '#ffffff', light: '#fbfcff', dark: '#f2f5fa' },
    bodyArea: { main: '#f5f7fb', light: '#eef2f8', dark: '#e3eaf4' },
    textPrimary: '#0f172a',
...
export const createDynamicTheme = (appearanceSettings: {
  ...
  const dividerColor = isForestTheme ? '#2d6a4f' : mode === 'dark' ? '#475569' : '#c6d1e0';
  ...
  const cardBorderColor = isForestTheme ? '#2d6a4f' : mode === 'dark' ? '#52647a' : '#d0d9e7';
  const cardHoverBorderColor = isForestTheme ? '#3b8f66' : mode === 'dark' ? '#64748b' : '#b8c6da';
  const cardShadow = mode === 'dark' ? '0 2px 8px rgba(2, 6, 23, 0.32)' : '0 1px 3px rgba(15, 23, 42, 0.08)';
  const cardHoverShadow = mode === 'dark' ? '0 4px 12px rgba(2, 6, 23, 0.4)' : '0 3px 10px rgba(15, 23, 42, 0.12)';
  const appBarBackground = mode === 'dark' ? '#253345' : '#fdfefe';
  const appBarTextColor = isForestTheme ? '#eafcf1' : mode === 'dark' ? '#f8fafc' : '#0f172a';
  const appBarBorderColor = isForestTheme ? '#2d6a4f' : mode === 'dark' ? '#52647a' : '#ccd6e4';
  const listHoverBg = isForestTheme ? 'rgba(34, 197, 94, 0.2)' : mode === 'dark' ? '#334155' : '#edf3ff';
  const listSelectedBg = isForestTheme ? 'rgba(34, 197, 94, 0.34)' : mode === 'dark' ? '#475569' : '#dbe7fd';
  const listSelectedHoverBg = isForestTheme ? 'rgba(74, 222, 128, 0.42)' : mode === 'dark' ? '#64748b' : '#c8daf8';
  ...
  return createTheme({
    ...
    typography: {
      ...
      h3: {
        fontSize: `${1.375 * fontSizeMultiplier}rem`,
        fontWeight: 600,
        lineHeight: 1.4,
        letterSpacing: '-0.025em',
      },
    ...
    components: {
      ...
      MuiTableCell: {
        styleOverrides: {
          ...
          head: {
            backgroundColor: isLightMode ? '#eef2f7' : '#2b3a4f',
      ...
      MuiDrawer: {
        styleOverrides: {
          paper: {
            ...
            backgroundColor: mode === 'dark' ? '#1b2738' : '#f8fafc',
            backgroundImage: 'none',
          },
        },
      },
